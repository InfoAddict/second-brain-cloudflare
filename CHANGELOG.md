...
- Searches that don't reuse a memory's own wording now find the right memory more often. After the usual keyword-and-meaning search picks its candidates, a second ranking step re-reads the closest ones against your question and moves the best match up. It only runs when the top results are close, never for exact names, versions or ticket numbers, and if it is ever unavailable, slow or gives a bad answer, search returns exactly what it returned before.
- The extra step uses a small slice of the free plan's daily AI allowance on the searches it runs. It turns itself on once a one-time self-check passes and repeats that check about weekly. `RERANK_MODE` (`off`, `on`, `auto`, default `auto`) is an admin setting in the config API; `off` restores the previous ranking exactly.
- Answers buried deep inside long notes are found. In our test set, a fact tucked into the middle of a 3,000-character note used to surface about one time in twelve; it now surfaces about one time in three, and short notes and exact-word searches are unaffected. Long memories are now indexed in small, labeled pieces, each remembering which note it came from, while the text you read and the keyword search stay exactly as you wrote them.
- Long notes take more room in the vector index, and how much you notice depends on how many long notes you keep. A note of about 3,000 characters now uses around seven small entries where it used three. The extra room is capped: once your index is about half full, new long notes go back to larger pieces, so on the free plan a brain where one memory in twenty is a long note fits about 8 percent fewer memories, one in five about 18 percent fewer, and even a brain where two in five are long notes about 24 percent fewer. Short notes use exactly the room they did.
- Saving a long note makes a few more small embedding calls (about seven for a 3,000-character note, where it made three), and capturing something that repeats a long note is still recognised as a duplicate.
- Existing brains upgrade by themselves, with no action and no downtime. Each hour a small batch of long notes is rewritten, using at most about a seventh of the day's free AI allowance, and picks up exactly where it left off if interrupted. A brain of 20,000 memories where one in five is long finishes in about two weeks, and most brains in a day or two. Search works the whole time: notes not yet rewritten keep working as before, and nothing you wrote is changed. Anyone in a hurry can run the rewrite on demand with `POST /migration/scheme`.
- An opt-in way to find answers buried deep inside long notes, off by default until it has been measured at scale. Turning on the `CONTEXTUAL_EMBEDDINGS` setting indexes long memories in small, labeled pieces, each remembering which note it came from, while the text you read and the keyword search stay exactly as you wrote them. On our test set a fact in the middle of a 3,000-character note went from surfacing about one time in twelve to about one time in three for the two search models we ship; on larger test brains the gain was smaller and not yet statistically clear, which is why it stays off. Nothing changes for you unless you turn it on: no migration runs, and the index and every request are exactly as before.
- If you turn it on: long notes take more index room, more the more of your memories are long (about seven small entries for a 3,000-character note instead of three, capped, and once the index is about half full new long notes return to larger pieces), and saving one makes a few more embedding calls. Existing memories are rewritten in the background, a small batch an hour, using at most about a seventh of the day's free AI allowance, resuming where it left off if interrupted; search works the whole time. `POST /migration/scheme` runs it on demand.
