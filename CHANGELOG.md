...
- Asking for more results no longer reshuffles the top ones. Search used to size its candidate pool, its diversity pass, and the result slots it reserves for linked memories by how many results you asked for, so the first five of a 10-result search could differ from a 5-result search. A 5-result search returns exactly what it did before; a larger request now only adds results after those five. Linked memories take fixed ranks (5th and 10th), so a request for fewer than 5 results includes none and one for 6 to 9 includes one.
