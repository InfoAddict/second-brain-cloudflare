...
- Long notes take more room in the vector index. A note of about 3,000 characters now uses around seven small entries where it used three, so a brain made mostly of long notes will fit fewer memories on the free plan; typical brains, where long notes are a small share, lose roughly a tenth of their headroom.
- Existing brains upgrade by themselves, with no action and no downtime. A nightly run rewrites a few long notes at a time (about a dozen pieces a night, most nights nothing at all) and picks up exactly where it left off if interrupted. Search works the whole time: notes not yet rewritten keep working as before. Anyone in a hurry can run the rewrite on demand with `POST /migration/scheme`. Nothing you wrote is changed.
