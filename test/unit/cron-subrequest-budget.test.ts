...
// MOVED 61 -> 67 (T-0042, embedding scheme migration): runSchemeBatch runs
// after FTS maintenance. Contextual embeddings ship on, so a brain upgraded
// from before them has work: the measured night is six operations, the
// resolveConfig GET, the scheme ledger GET, the model-migration GET, the page
// and count SELECTs, and the ledger PUT (an empty brain, so no entry is
...
const NIGHTLY_D1_STATEMENT_BUDGET = 67;
...
describe("nightly cron D1 subrequest cost", () => {
  ...
  it("keeps an ordinary night (no dangling-edge sweep) inside the free-plan D1 budget", async () => {
    ...
    expect(statements.length).toBe(28); // 22 + 6 scheme-migration ops on a brain with work (T-0042)
  });
  ...
  it("keeps a sweep night (the weekly dangling-edge sweep runs) inside the free-plan D1 budget", async () => {
    ...
    expect(statements.length).toBe(29); // 23 + 6 scheme-migration ops on a brain with work (T-0042)
  });
  ...
  it("keeps a ready night (the integrity checks and the rotating window run) inside the budget", async () => {
    ...
    expect(statements.length).toBe(30); // 24 + 6 scheme-migration ops on a brain with work (T-0042)
  });
