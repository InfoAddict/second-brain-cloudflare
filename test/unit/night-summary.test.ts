...
describe("recordNightSummary", () => {
  it("writes one full record under night:<workspaceId>", async () => {
    ...
    await recordNightSummary(env, "ws-a", {
      linksInferred: 3, insightsProposed: 0, digestsWritten: 1, claimsFlagged: 5, whenJudged: 0, whenExtracted: 0,
    });
  ...
  it("keys each workspace's record separately", async () => {
    ...
    await recordNightSummary(env, "ws-a", { linksInferred: 1, insightsProposed: 0, digestsWritten: 0, claimsFlagged: 0, whenJudged: 0, whenExtracted: 0 });
    await recordNightSummary(env, "ws-b", { linksInferred: 9, insightsProposed: 0, digestsWritten: 0, claimsFlagged: 0, whenJudged: 0, whenExtracted: 0 });
  ...
  it("swallows a failing put and never throws", async () => {
    ...
    await expect(recordNightSummary(env, "ws-a", {
      linksInferred: 1, insightsProposed: 0, digestsWritten: 0, claimsFlagged: 0, whenJudged: 0, whenExtracted: 0,
