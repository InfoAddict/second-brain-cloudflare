...
describe("GET /brief", () => {
  ...
  it("surfaces patterns awaiting a decision, and skips dismissed ones", async () => {
    sq = await migrated();
    const now = Date.now();
    sq.seed({ id: "p1", content: "You keep deferring the pricing decision", createdAt: now - HOUR, tags: ["auto-pattern"] });
    sq.seed({ id: "p2", content: "Dismissed already", createdAt: now - HOUR, tags: ["auto-pattern", "status:deprecated"] });
