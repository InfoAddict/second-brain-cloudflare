...
function buildMcpServer(env: Env): McpServer {
  ...
  server.tool(
    ...
    async ({ content, tags, source }) => {
      ...
      try {
        ...
        await env.VECTORIZE.insert([{ id, values, metadata: { content: c.slice(0, 512), tags: tags ?? [], source: source ?? "claude", created_at: now } }]);
      } catch (e) { console.error("Vectorize:", e); }
  ...
  server.tool(
    ...
    async ({ id }) => {
      ...
      try { await env.VECTORIZE.deleteByIds([id]); } catch (e) { console.error("Vectorize:", e); }
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/capture" && request.method === "POST") {
      ...
      ctx.waitUntil(
        embed(c, env)
          .then((values) => env.VECTORIZE.insert([{ id, values, metadata: { content: c.slice(0, 512), tags: body.tags ?? [], source: body.source ?? "api", created_at: now } }]))
    ...
    if (url.pathname === "/mcp") {
      ...
      // Handle request and keep server alive until response is consumed
      const response = await transport.handleRequest(request);

      // Don't close server until response body is fully sent
      ctx.waitUntil(
